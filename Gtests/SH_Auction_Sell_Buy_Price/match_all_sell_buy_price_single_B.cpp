...
// 实盘下推送行情，全部成交，买单，买一价1.050元，卖一价1.045元 ，不验股，  
//aSHShare.account = "A645078963";	
//aSHShare.stock = "600302" ;	标准股份
//			SingleGtestMatchAllWithQuotation_B.SellBuyPrice_1
TEST(SingleGtestMatchAllWithQuotation_B, SellBuyPrice_1)
{
	int iRes = 0;
	//切换模式
...
// 实盘下推送行情，全部成交，买单，买一价1.050元，卖一价1.045元， 验股，  
//			SingleGtestMatchAllWithQuotation_B.SellBuyPriceCheckAsset_3
TEST(SingleGtestMatchAllWithQuotation_B, SellBuyPriceCheckAsset_3)
{
	int iRes = 0;
	//切换模式
	ASSERT_EQ(0, TransformMatchMode(SellBuyPrice));
	...
	aSHShare.reff.replace(10 - strlen(szTemp), strlen(szTemp), szTemp);
	aSHShare.rec_num = szTemp;
	aSHShare.price = "1.050";				//卖一价是1.045
	aSHShare.qty = "100";
	aSHShare.bs = "B";					//买
